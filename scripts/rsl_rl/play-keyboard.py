...
def main():
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            # [新增 2] 读取键盘输入并构造指令
            # advance() 返回 [x, y, z, roll, pitch, yaw] 的变化量
            delta_cmd = keyboard.advance().to(env.device)

            # 构造用户指令张量，形状必须与 commands 一致 [num_envs, 3]
            # 假设 commands 是 3维: (vx, vy, omega_z)
            # 如果你的 commands 是 4维 (vx, vy, vz, omega_z)，请自行调整索引
            user_commands = torch.zeros_like(commands) 
            
            # 映射键盘操作到速度指令
            user_commands[:, 0] = delta_cmd[:, 0]  # W/S -> vx (前后)
            user_commands[:, 1] = delta_cmd[:, 1]  # A/D -> vy (左右)
            user_commands[:, 2] = delta_cmd[:, 5]  # Q/E -> omega_z (转向)
            ...
            # [注意] 这里从 info 里拿到的 commands 是环境下一帧生成的随机指令
            # 在下一次循环开头，它会被我们的 user_commands 再次覆盖
